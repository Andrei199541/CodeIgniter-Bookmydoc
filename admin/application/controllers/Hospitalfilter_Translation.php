...
class Hospitalfilter_Translation extends CI_Controller {
	public function __construct() {
	parent::__construct();
		date_default_timezone_set("Asia/Kolkata");
		$this->load->model('HospitalfilterTranslation_model');
		$this->load->helper('directory');
		if(!$this->session->userdata('logged_in')) { 
			redirect(base_url());
		}
    }
	public function add_hospitalfilter()
	{
		$template['page'] = "Translation/Hospitalfilter/add_hospitalfilter";
		$template['page_title'] = "Add Language Translation";
		$this->load->view('template',$template);
	}
	public function insert_hospitalfilter()
	{
		$data= json_decode(file_get_contents("php://input"));
		$data =(array) $data;
        $role=$this->HospitalfilterTranslation_model->inserthome($data);
        echo $role;die;
	}
	public function view_hospitalfilter()
	{		
		$textFile ='hospitalfilter_lang.php';
		...
		endforeach;
		$template['final_lang'];
		$template['page'] = "Translation/Hospitalfilter/view_hospitalfilter";
		$template['page_title'] = "View Language Translation";		
		$this->load->view('template',$template);	
	}
	public function edit_hospitalfilter()
	{
		$template['language_name'] = $this->uri->segment(3);
		$template['page'] = "Translation/Hospitalfilter/edit_hospitalfilter";
		$template['page_title'] = "edit Language Translation";
		$this->load->view('template',$template);
			
	}
	public function hospitalfilter_delete()
	{
		$textFile ='hospitalfilter_lang.php';
		...
		if(unlink($locationfiles)):
			...
			if(is_bool($lang_directory ) === false):
				...
				$result = $this->HospitalfilterTranslation_model->delete_language($language_name);				
				if($result):
					$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
					redirect(base_url().'Hospitalfilter_Translation/view_hospitalfilter');
				endif;		
			else:
				$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
				redirect(base_url().'Hospitalfilter_Translation/view_hospitalfilter');	
