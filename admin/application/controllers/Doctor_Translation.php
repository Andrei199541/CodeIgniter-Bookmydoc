...
class Doctor_Translation extends CI_Controller {
	public function __construct() {
	parent::__construct();
		date_default_timezone_set("Asia/Kolkata");
		$this->load->model('DoctorTranslation_model');
	...
	{
		$template['page'] = "Translation/Doctor/add_doctor";
	...
	public function insert_doctor()
	{
		...
		$data =(array) $data;
        $role=$this->DoctorTranslation_model->inserthome($data);
        echo $role;die;
	}
	public function view_doctor()
	{		
		$textFile ='doctor_lang.php';
		...
		endforeach;
		$template['final_lang'];
		$template['page'] = "Translation/Doctor/view_doctor";
	...
	public function edit_doctor()
	{
		...
		$template['page'] = "Translation/Doctor/edit_doctor";
	...
	public function medicalprofile_delete()
	{
		...
		if(unlink($locationfiles)):
			...
			if(is_bool($lang_directory ) === false):
				...
				$result = $this->DoctorTranslation_model->delete_language($language_name);				
				if($result):
					$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
					redirect(base_url().'Doctor_Translation/view_doctor');
				endif;		
			else:
				$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
				redirect(base_url().'Doctor_Translation/view_doctor');	
