...
class Medicalfilter_Translation extends CI_Controller {
	public function __construct() {
	parent::__construct();
		date_default_timezone_set("Asia/Kolkata");
		$this->load->model('MedicalfilterTranslation_model');
		$this->load->helper('directory');
		if(!$this->session->userdata('logged_in')) { 
			redirect(base_url());
		}
    }
	public function add_medicalfilter()
	{
		$template['page'] = "Translation/Medicalfilter/add_medicalfilter";
		$template['page_title'] = "Add Language Translation";
		$this->load->view('template',$template);
	}
	public function insert_medicalfilter()
	{
		$data= json_decode(file_get_contents("php://input"));
		$data =(array) $data;
        $role=$this->MedicalfilterTranslation_model->inserthome($data);
        echo $role;die;
	}
	public function view_medicalfilter()
	{		
		$textFile ='medicalfilter_lang.php';
		...
		endforeach;
		$template['final_lang'];
		$template['page'] = "Translation/Medicalfilter/view_medicalfilter";
		$template['page_title'] = "View Language Translation";		
		$this->load->view('template',$template);	
	}
	public function edit_medicalfilter()
	{
		$template['language_name'] = $this->uri->segment(3);
		$template['page'] = "Translation/Medicalfilter/edit_medicalfilter";
		$template['page_title'] = "edit Language Translation";
		$this->load->view('template',$template);
			
	}
	public function medicalfilter_delete()
	{
		$textFile ='medicalfilter_lang.php';
		...
		if(unlink($locationfiles)):
			...
			if(is_bool($lang_directory ) === false):
				...
				$result = $this->MedicalfilterTranslation_model->delete_language($language_name);				
				if($result):
					$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
					redirect(base_url().'Medicalfilter_Translation/view_medicalfilter');
				endif;		
			else:
				$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
				redirect(base_url().'Medicalfilter_Translation/view_medicalfilter');	
