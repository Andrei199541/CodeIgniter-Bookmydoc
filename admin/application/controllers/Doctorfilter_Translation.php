...
class Doctorfilter_Translation extends CI_Controller {
	public function __construct() {
	parent::__construct();
		date_default_timezone_set("Asia/Kolkata");
		$this->load->model('DoctorfilterTranslation_model');
		$this->load->helper('directory');
		if(!$this->session->userdata('logged_in')) { 
			redirect(base_url());
		}
    }
	public function add_doctorfilter()
	{
		$template['page'] = "Translation/Doctorfilter/add_doctorfilter";
		$template['page_title'] = "Add Language Translation";
		$this->load->view('template',$template);
	}
	public function insert_doctorfilter()
	{
		$data= json_decode(file_get_contents("php://input"));
		$data =(array) $data;
        $role=$this->DoctorfilterTranslation_model->inserthome($data);
        echo $role;die;
	}
	public function view_doctorfilter()
	{		
		$textFile ='doctorfilter_lang.php';
		...
		endforeach;
		$template['final_lang'];
		$template['page'] = "Translation/Doctorfilter/view_doctorfilter";
		$template['page_title'] = "View Language Translation";		
		$this->load->view('template',$template);	
	}
	public function edit_doctorfilter()
	{
		$template['language_name'] = $this->uri->segment(3);
		$template['page'] = "Translation/Doctorfilter/edit_doctorfilter";
		$template['page_title'] = "edit Language Translation";
		$this->load->view('template',$template);
			
	}
	public function doctorfilter_delete()
	{
		$textFile ='doctorfilter_lang.php';
		...
		if(unlink($locationfiles)):
			...
			if(is_bool($lang_directory ) === false):
				...
				if($result):
					...
					redirect(base_url().'Doctorfilter_Translation/view_doctorfilter');
				endif;		
			else:
				$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
				redirect(base_url().'Doctorfilter_Translation/view_doctorfilter');	
