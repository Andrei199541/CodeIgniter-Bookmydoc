...
class Terms_Translation extends CI_Controller {
	public function __construct() {
	parent::__construct();
		date_default_timezone_set("Asia/Kolkata");
		$this->load->model('TermsTranslation_model');
	...
	{
		$template['page'] = "Translation/Terms/add_terms";
	...
	public function insert_terms()
	{
		...
		$data =(array) $data;
        $role=$this->TermsTranslation_model->inserthome($data);
        echo $role;
	}
	public function view_terms()
	{		
		$textFile ='terms_lang.php';
		...
		endforeach;
		$template['final_lang'];
		$template['page'] = "Translation/Terms/view_terms";
	...
	public function edit_terms()
	{
		...
		$template['page'] = "Translation/Terms/edit_terms";
	...
	public function terms_delete()
	{
		...
		if(unlink($locationfiles)):
			...
			if(is_bool($lang_directory ) === false):
				...
				$result = $this->TermsTranslation_model->delete_language($language_name);				
				if($result):
					$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
					redirect(base_url().'Terms_Translation/view_terms');
				endif;		
			else:
				$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
				redirect(base_url().'Terms_Translation/view_terms');	
