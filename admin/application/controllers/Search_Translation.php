...
class Search_Translation extends CI_Controller {
	public function __construct() {
	parent::__construct();
		date_default_timezone_set("Asia/Kolkata");
		$this->load->model('SearchTranslation_model');
	...
	{
		$template['page'] = "Translation/Search/add_search";
	...
	public function insert_search()
	{
		...
		$data =(array) $data;
        $role=$this->SearchTranslation_model->inserthome($data);
        echo $role;
	}
	public function view_search()
	{		
		$textFile ='search_lang.php';
		...
		endforeach;
		$template['final_lang'];
		$template['page'] = "Translation/Search/view_search";
	...
	public function edit_search()
	{
		...
		$template['page'] = "Translation/Search/edit_search";
	...
	public function search_delete()
	{
		...
		if(unlink($locationfiles)):
			...
			if(is_bool($lang_directory ) === false):
				...
				$result = $this->SearchTranslation_model->delete_language($language_name);				
				if($result):
					$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
					redirect(base_url().'Search_Translation/view_search');
				endif;		
			else:
				$this->session->set_flashdata('message', array('message' => 'Deleted Successfully','class' => 'success'));
				redirect(base_url().'Search_Translation/view_search');	
