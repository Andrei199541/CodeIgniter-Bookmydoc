	...
	function inserthome($data){
		...
		if(isset($updated_id)){
			...
			fclose($terms_file);
			 ...
			 $booking_lang=$location.$folderName.'/'.'booking_lang'.$extension;
			$booking_file = fopen($booking_lang, "wb") or die("Unable to open file!");				
			$booking_txt = '<?php ';
			foreach($bookingdata as $key=>$value){
				$booking_txt .='$'.$key.' = "'.$value.'";'."\r\n";
			}
			$booking_txt .=' ?>';
			fwrite($booking_file, $booking_txt);
			fclose($booking_file);
