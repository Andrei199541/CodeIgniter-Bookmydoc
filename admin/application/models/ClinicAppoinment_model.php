<?php 
class ClinicAppoinment_model extends CI_Model {	
	public function _consruct(){
		parent::_construct();
 	}		
	function get_appoinmentdetails()	{
			$this->db->select('appointment.id as id, appointment.*, patient.patient_firstname,clinic.clinic_name, doctor.doctor_firstname');          
			$this->db->from('appointment' );
			$this->db->join('patient', 'patient.id = appointment.patient_id');
			$this->db->join('clinic', 'clinic.id = appointment.clinic_id');
	...
	function clinicappointmentsinfo_add($data){
	...
	public function get_clinicname(){
	...
	function get_appoinment_clinic($id){
		$this->db->where('id', $id);
	...
	function view_doctor($id){
		...
		$this->db->where(sprintf("find_in_set('%d', clinic_id) !=",$id), 0);
		$query = $this->db->get('doctor');
		$result = $query->result();
		return $result;	
	}
function get_doctornameindividual($id){
	...
	function get_doctortotalindividual($id){
		...
		$result1 = $query->row();
		if($result1){
			$this->db->where(sprintf("find_in_set('%d', clinic_id) !=",$result1->clinic_id), 0);
			$query = $this->db->get('doctor');
			$result = $query->result();
			if($result){
				return $result;		
