<div class="content-wrapper" id="doctor_edit_wrapper" target="<?php //echo $data['id'];?>" onreadystatechange="test();">
        ...
        <section class="content">
		  <div class="row">
			...
			$file_response= $this->session->flashdata('statusMsg');
			?>
			<?php $response =  $this->session->flashdata('msg');?>
			...
			<?php }?>
			<?php if(!empty($file_response)) {?>
				<div class="alert alert-success alert-dismissible">
				<button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
				<h4><i class="icon fa fa-check"></i> Alert!</h4>
				<?php echo $file_response;?>
				</div>
			<?php }?>
			<?php if(isset($response) && $response==false) {?>
				...
				Sorry You can't do this first fill user profile details.
				</div>
			<?php } ?>
			<?php if(!empty($error)) {?>
				<div class="alert alert-danger alert-dismissible">
			...
			<?php } ?>
		      	...
		      	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
	                	<div class="box box-info">
				            ...
				            <!-- /.box-header -->
				            <!-- form start -->
				            <!--<form class="form-horizontal" name="doc_work_time" id="doc_work_time" method="post" action="">-->						
	                       <form class="form-horizontal" name="doc_work_time"  id="doc_work_time" method="post" action="<?php echo base_url();?>Doctor_ctrl/addworking_time/">
				              <div class="box-body">
				              	...
				              	<?php foreach ($Days as $key => $value) { ?>
					                  ...
					                  <div class="col-sm-4">
					                    <input type="text" placeholder="Start Time" id="<?php echo $value.'_working_start';?>" required value="<?php echo (!empty($data['working_time'])) ? isset($working_time[ $days[$key]]['start']) ? $working_time[ $days[$key]]['start'] :'' : '';?>" name="work[<?php echo $days[$key];?>][start]" class="form-control timepicker start">
					                  </div>
					                  <div class="col-sm-4">
					                    <input type="text" placeholder="End Time" id="<?php echo $value.'_working_end';?>" required value="<?php echo (!empty($data['working_time'])) ? isset($working_time[ $days[$key]]['end']) ? $working_time[ $days[$key]]['end'] :'' : '';?>"  name="work[<?php echo $days[$key];?>][end]" class="form-control timepicker end">
					                  </div>
	                	...
	                	<div class="box box-info">
				            ...
				            <form class="form-horizontal" name="doctor_break_time"  id="doctor_break_time"  method="post" action="">
				              <div class="box-body">
				                ...
				                <?php foreach ($break_time[$days[$key]] as $br_key => $breaktime) { ?>
					                  ...
					                  <div class="col-sm-3">
					                    <input type="text"  placeholder="Start Time" id="<?php echo $value.'_break_start';?>" <?php echo  (empty($data['break_time'])) ? 'disabled' :'';?> value="<?php echo (!empty($data['break_time']) && !empty($data['break_time'])) ? isset($breaktime['start']) ? $breaktime['start'] :'' : '';?>"  name="break[<?php echo $days[$key];?>][start][]" class="form-control start <?php echo $days[$key].'_break';?>">
					                  </div>

					                  <div class="col-sm-3">
					                    <input type="text"  placeholder="End Time" id="<?php echo $value.'_break_end';?>" <?php echo  (empty($data['break_time'])) ? 'disabled' :'';?> value="<?php echo (!empty($data['break_time']) && !empty($data['break_time'])) ? isset($breaktime['end']) ? $breaktime['end'] :'' : '';?>" name="break[<?php echo $days[$key];?>][end][]" class="form-control end <?php echo $days[$key].'_break';?>">
					                  </div>
					                  <label class="col-sm-1 control-label" for="inputEmail3">
							...
							<div class="overlay"><p>sorry you need to fill user details first</p></div>
							<?php } ?>
							<?php if(empty($data['working_time']) && !empty($data['id'])){ ?>
							<div class="overlay"><p>sorry you need to fill working time</p></div>
							<?php } ?>
		          		</div>
			        </div>
			    <!-- vacation details -->
			    <div class="row">
		            ...
		            <!-- /.box-header -->
		            <!-- form start -->
		            <form role="form" name="doctor_vacation_time"  id="doctor_vacation_time"  method="post" action="">
		              <div class="box-body">
		              		...
		              		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				              	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
				                	<div class="form-group">
				                  		...
				                  		<input type="text"  data-inputmask="'alias': 'yyyy-mm-dd'" data-mask=""  placeholder="Start Date" required name="startdate[]" id="exampleInputEmail1" value="<?php echo $value['startdate'];?>" class="form-control start_date">
				                	</div>
				              	</div>
				              	<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
				                	<div class="form-group">
				                  		<label for="exampleInputEmail1">End Date</label>
				                  		<input type="text"  data-inputmask="'alias': 'yyyy-mm-dd'" data-mask=""  placeholder="End Date" required name="enddate[]"  id="exampleInputEmail1" value="<?php echo $value['enddate'];?>"  class="form-control end_date">
				                	</div>
				              	</div>
				            </div>
